...
public class DraftPopuWindow extends PopupWindow {
    ...
    private void init() {
        mContentView = LayoutInflater.from(context).inflate(R.layout.draft_status_popuwindow, null);
        RxView.clicks(mContentView.findViewById(R.id.dt_show))
                .throttleFirst(Constants.ClickDuration, TimeUnit.SECONDS)
                .subscribe(aVoid -> {
                    if (mClick != null) {
                        mClick.onPublish(true);
                    }
                    dismiss();
                });
    ...
    public interface ShowClickListener {
        void onPublish(boolean ispublish);
