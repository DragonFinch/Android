...
import com.iyoyogo.android.eventbus.MessageEvent;
...
public class ForeigniMapFragment extends BaseFragment<MapContract.Presenter> implements AbsListView.OnScrollListener,MapContract.View {
    ...
    protected List<MapBean.DataBean.ListBean> totalCityList = new ArrayList<>();
    ...
    public void onResume() {
        ...
        String user_id = SpUtils.getString(getActivity(), "user_id", null);
        String user_token = SpUtils.getString(getActivity(), "user_token", null);
        mPresenter.aboutMe(user_id,user_token,"internal","石家庄");
        Log.e("onResume", "onResume: "+user_id +"!!!!!!!"+user_token );
    }*/

    @Override
    protected int getLayoutId() {
        return R.layout.mapguomei;
    }
    ...
    private void showSetCityDialog(String name, String cityCode) {
        ...
        builder.setPositiveButton("确定", new DialogInterface.OnClickListener() { //设置确定按钮
            ...
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
                Log.e("eventbus", "onClick1111111: "+name );
                //选中之后做你的方法
                // ;
                EventBus.getDefault().post(new MessageEvent("1"));
                Log.e("eventbus", "onClick: "+name);
    ...
    private void initData1() {
        ...
        totalCityLv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (position > 1) {
                    MapBean.DataBean.ListBean listBean = totalCityList.get(position);

                    showSetCityDialog(listBean.getChina_name(), listBean.getCode());
                }
    ...
    public void initTotalCityList( ) {
        ...
        try {
            ...
            JSONArray array = jsonObject.getJSONArray("Cityy");
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        ...
        if (mReady) {
            String key = getAlpha(hotCityList.get(firstVisibleItem).getKey());
            overlay.setText(key);
    ...
    public void map(MapBean map) {
        Log.e("asdasdasdass", "map: "+map );
        list = map.getData().getList();
        //
    }
    ...
    private class CityListAdapter extends BaseAdapter {
        ...
        private List<MapBean.DataBean.ListBean> totalCityList;
        private List<CityEntity> hotCityList;
        private LayoutInflater inflater;
        final int VIEW_TYPE = 3;

        CityListAdapter(Context context,
                        List<MapBean.DataBean.ListBean> totalCityList,
            ...
            for (int i = 0; i < totalCityList.size(); i++) {
                // 当前汉语拼音首字母
                String currentStr = hotCityList.get(i).getFirst();

                String previewStr = (i - 1) >= 0 ? hotCityList.get(i - 1).getFirst() : " ";
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (viewType == 0) { // 定位
                ...
                holder.cityKeyTv.setText(getAlpha(hotCityList.get(position).getKey()));
                holder.cityNameTv.setText(listBean.getChina_name());

                if (position >= 1) {
                    MapBean.DataBean.ListBean listBean1 = totalCityList.get(position - 1);
                    if (hotCityList.get(position).getKey().equals(hotCityList.get(position).getKey())) {
                        holder.cityKeyTv.setVisibility(View.GONE);
                    } else {
                        holder.cityKeyTv.setVisibility(View.VISIBLE);
                    }
