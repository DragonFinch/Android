...
//评论消息
public class CommentMessageAdapter extends BaseQuickAdapter<MessageBean.DataBean.ListBean, BaseViewHolder> {
    public CommentMessageAdapter(int layoutResId, @Nullable List<MessageBean.DataBean.ListBean> data) {
    ...
    protected void convert(BaseViewHolder helper, MessageBean.DataBean.ListBean item) {
        ...
        //comment_message_user_icon头像，comment_message_user_nick_name橘色文字，comment_message_tv_action橘色旁边黑色文字
        //comment_message_tv_reply回复按钮，comment_message_tv_content下边评论信息，comment_message_tv_time时间，comment_message_item_like_iv_id评论的图片
        TextView comment_message_tv_reply = helper.getView(R.id.comment_message_tv_reply);
        boolean is_reply = item.isIs_reply();
        if (is_reply) {
            comment_message_tv_reply.setVisibility(View.VISIBLE);
        } else {
            comment_message_tv_reply.setVisibility(View.GONE);
        }
        helper.setText(R.id.comment_message_user_nick_name, item.getUser_nickname());
        helper.setText(R.id.comment_message_tv_action, item.getTitle());
        helper.setText(R.id.comment_message_tv_content, item.getContent());
        helper.setText(R.id.comment_message_tv_time, item.getCreate_time());
        RequestOptions requestOptions1 = new RequestOptions();
        requestOptions1.placeholder(R.mipmap.default_touxiang);
        requestOptions1.error(R.mipmap.default_touxiang);
        Glide.with(mContext).load(item.getUser_logo()).apply(requestOptions1).into((CircleImageView) helper.getView(R.id.comment_message_user_icon));
        RequestOptions requestOptions2 = new RequestOptions();
        requestOptions1.placeholder(R.mipmap.default_ic);
        requestOptions1.error(R.mipmap.default_ic);
        Glide.with(mContext).load(item.getFile_path()).apply(requestOptions2).into((ImageView) helper.getView(R.id.comment_message_item_like_iv_id));
        CircleImageView user_icon = helper.getView(R.id.comment_message_user_icon);
