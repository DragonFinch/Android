...
public class CustomAnimateStickerEffectActivity extends BaseActivity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        switch (id){
            case R.id.download_more_btn:
                ...
                AppManager.getInstance().jumpActivityForResult(AppManager.getInstance().currentActivity(), CustomStickerEffectDownloadActivity.class, null,CUSTOMSTICKERREQUESTLIST);
                break;
    ...
    private void initCustomStickerEffectDataList(){
        ...
        ArrayList<NvAsset> userableAsset = getCustomAssetsDataList();
        if(userableAsset != null && userableAsset.size() > 0) {
            String bundlePath =  "customsticker/info.txt";
            Util.getBundleFilterInfo(this, userableAsset, bundlePath);
            for(NvAsset asset:userableAsset) {
                if(asset.isReserved()){
                    String coverPath = "file:///android_asset/customsticker/";
                    coverPath += asset.uuid;
                    coverPath += ".png";
                    asset.coverUrl = coverPath;//加载assets/sticker文件夹下的图片
                }
            }
            mCustomStickerAssetList = userableAsset;
        }
    }
