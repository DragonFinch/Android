...
public class LabelTwoAdapter extends PagerAdapter {
    private Context mContext;
    private List<LabelListBean.DataBean.List2Bean> labels;
    public TagAdapter<LabelListBean.DataBean.List2Bean> tagAdapter;
    static ArrayList<LabelListBean.DataBean.List2Bean> mSelectImg=new ArrayList<>();

    //值得做自定义：AC  防坑值得做BC 我的专属C
    public LabelTwoAdapter(Context mContext, List<LabelListBean.DataBean.List2Bean> labels) {
        this.mContext = mContext;
        this.labels = labels;

    }

    public void setLabels(List<LabelListBean.DataBean.List2Bean> labels) {
        this.labels = labels;
...
//        notifyDataSetChanged();
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        ...
        if (tagAdapter == null) {
            tagAdapter = new TagAdapter<LabelListBean.DataBean.List2Bean>(labels) {
                @Override
                public View getView(FlowLayout parent, int position, LabelListBean.DataBean.List2Bean list1Bean) {

                    View contentView = LayoutInflater.from(mContext).inflate(R.layout.item_label_two, tagFlowLayout, false);
    ...
    public ArrayList<LabelListBean.DataBean.List2Bean> selectSign(){
